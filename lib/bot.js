...
const commandList = [
    ...
    { name: '!roll <max>, !roll <min> <max>', value: 'Roll a number between 1 and <max> or between <min> and <max>'}
]
...
const doRoll = async (msg, args) => {
	...
	if (args.length === 2) {
		...
		msg.reply("Usage: !roll <max> or !roll <min> <max>")
		return
	}
	...
	if (num2) {
		if (num1 < num2) {
			msg.reply(roll2(num1, num2))
		} else {
			msg.reply(roll2(num2, num1))
		} 
	} else {
		msg.reply(1 + roll1(num1))
	}
}
